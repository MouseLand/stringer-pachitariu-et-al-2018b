...
for wnorm = 0:1
	if wnorm==0
		...
		results = load(fullfile(matroot,'gabor_fits_wnorm.mat'));
	end
	...
	for K = 1:7
		...
		respG2 = max(0, respG2);
		if results.wnorm
			isnorm = logical(bestGB(:,2));
			respG1(isnorm, :) = respG1(isnorm, :) ./ mean(respG1.^2,1).^.5;
			respG2(isnorm, :) = respG2(isnorm, :) ./ mean(respG2.^2,1).^.5;
		end
	...
	if wnorm==0
		...
		save(fullfile(matroot,'gabor_spectrum_wnorm.mat'),'specS','alpha');
	end
end
