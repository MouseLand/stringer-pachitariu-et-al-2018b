...
%% choose example dataset and compute receptive fields
...
hp.Position(2) = hp.Position(2);%-.08;
...
for k = 1:numel(snr{1})
	if k == 1
		text(.6, 1.1 - .1*k, sprintf('N = %d neurons', numel(vexpALL{k})),...
			'Color', col(icol(k),:), 'Fontangle', 'normal', 'Fontsize', 6);
	else
		text(.6, 1.1 - .1*k, sprintf('N = %d', numel(vexpALL{k})),...
			'Color', col(icol(k),:), 'Fontangle', 'normal', 'Fontsize', 6);
	end
end
...
for k = 1:size(pCorrect,2)
	...
	hold all
	%text(.1, .35 + (10-k)*.07, sprintf('M%d', k), 'Color', col(icol(k), :), 'Fontweight', 'bold')
end
